...
interface ListApprovePropertyProps {
  properties: Property[];
}

const ListApproveProperty: React.FC<ListApprovePropertyProps> = ({ properties }) => {
  ...
  const [sortBy, setSortBy] = useState<'building_name' | 'createdAt' | 'sale_status'>('createdAt');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [selectedProperty, setSelectedProperty] = useState<Property | null>(null);
  const [viewDialogOpen, setViewDialogOpen] = useState(false);
  const [approveDialogOpen, setApproveDialogOpen] = useState(false);
  const [approveLoading, setApproveLoading] = useState(false);
  const [propertyToApprove, setPropertyToApprove] = useState<string | null>(null);
  ...
  const handleApproveProperty = async () => {
    ...
    try {
      await axiosInstance.patch(`/api/admin/properties/${propertyToApprove}/activate`);
      setApproveDialogOpen(false);
      setPropertyToApprove(null);
      window.location.reload();
    } catch (err: any) {
      console.error('Failed to approve property:', err);
      alert(err?.response?.data?.message || err?.message || 'Failed to approve property');
    } finally {
  ...
  return (
    <>
      <Card sx={{ p: 3, mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Typography variant="h6" sx={{ fontWeight: 600 }}>
            Properties Pending Approval ({filteredProperties.length})
          </Typography>
          <TextField
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to approve this property? This will activate the property and make it visible to all users.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
...
export default ListApproveProperty;
