...
const UpdateBusinessDetailsForm: React.FC<UpdateBusinessDetailsFormProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.patch(`/agent/properties/${propertyId}/business-details`, formData);
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to update business details');
      }
    } catch (error: any) {
    ...
    <Box>
      <form onSubmit={handleSubmit}>
        ...
        <Box sx={{ mt: 4, display: 'flex', justifyContent: 'flex-end' }}>
          ...
          >
            {isSubmitting ? 'Updating...' : isSubmitted ? 'Business Details Updated' : 'Update Business Details'}
          </Button>
        </Box>
      </form>
    </Box>
