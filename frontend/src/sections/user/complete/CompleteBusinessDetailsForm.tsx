...
const BusinessDetailsForm: React.FC<BusinessDetailsFormProps> = ({ propertyId, onStepSubmitted, initialData, onDataChange }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData(prev => ({
        ...prev,
        business_rates: initialData.business_rates || prev.business_rates,
        descriptions: initialData.descriptions || prev.descriptions,
        sale_types: initialData.sale_types || prev.sale_types,
      }));
    }
  }, [initialData]);
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        {(!formData.sale_types || formData.sale_types.length === 0) && (
          <Box>
