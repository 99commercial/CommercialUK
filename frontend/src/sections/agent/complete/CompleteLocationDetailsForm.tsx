...
const CompleteLocationDetailsForm: React.FC<LocationDetailsFormProps> = ({ propertyId, onStepSubmitted, initialData, onDataChange }) => {
  const {
    formState: { errors },
    setValue,
  ...
  useEffect(() => {
    if (initialData) {
      setFormData(prev => ({
        ...
        address_details: initialData.address_details || prev.address_details,
        map_settings: initialData.map_settings || prev.map_settings,
        geocoding_info: initialData.geocoding_info || prev.geocoding_info,
        location_verified: initialData.location_verified || prev.location_verified,
        verification_notes: initialData.verification_notes || prev.verification_notes,
      }));
    }
  }, [initialData]);
  ...
  const handleSave = async () => {
    ...
    try {
      let response = await axiosInstance.put(`/api/agent/properties/${propertyId}/location`, formData);
...
export default CompleteLocationDetailsForm;
